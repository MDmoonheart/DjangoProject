...
def create_dataset(data_path, batch_size=24, repeat_num=1):
    data_set = ds.ImageFolderDataset(data_path, num_parallel_workers=6, shuffle=True)
...
#         self.end_point = _fc(out_channels[3], num_classes, use_se=self.use_se)
        self.end_point1 = _fc(out_channels[3], 100)
        self.dropout1 = nn.Dropout(keep_prob=0.5)
        self.end_point2 = _fc(100, 50)
        self.dropout2 = nn.Dropout(keep_prob=0.3)
        self.end_point3 = _fc(50, num_classes)
    ...
    def construct(self, x):
        ...
        out = self.end_point1(out)
        out = self.leakyReLU(out)
        out = self.dropout1(out)
        out = self.end_point2(out)
        out = self.leakyReLU(out)
        out = self.dropout2(out)
        out = self.end_point3(out)
        #out = out[0][1:1]
        return out

class ResNet_(nn.Cell):
    """
    ResNet architecture.
...
class ResNet_score(nn.Cell):
    ...
    def __init__(self,
                 ...
                 use_se=False,
                 res_base=False):
        super(ResNet_score, self).__init__()
...
def resnet50_score(class_num=10):
    return ResNet_(ResidualBlock,
                  [3, 4, 6, 3],
                  [64, 256, 512, 1024],
                  [256, 512, 1024, 2048],
